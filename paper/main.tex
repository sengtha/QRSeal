...
The described mechanism is online lookup: the code is scanned with an ordinary
camera or browser, the platform is consulted, and it \emph{displays the
authenticated original document} for the reader to compare against the paper in
hand. Profile B is offline verification: the code carries a signature, and the
verifier checks it against a trust list without contacting anyone. Our
implementation enforces the difference with a test asserting that verification
performs no network access at all.

\paragraph{Where online lookup is straightforwardly better.} It defeats the
transplant attack, which Profile B can only mitigate. We ask a verifier to
compare signed fields against the printed page and constrain the API so the
comparison cannot be skipped; an online lookup \emph{supplies} the authoritative
original to compare with. That is a stronger answer, and we would rather record
it than pretend the trade runs one way.
...
\paragraph{On the ledger.} The platform is described as inscribing a
cryptographic hash of each document on a blockchain layer
\citep{cambodianess2026verifygov}. That is a real property and it is worth being
...
website. A rule phrased as ``an official QR code never opens a website'' would
