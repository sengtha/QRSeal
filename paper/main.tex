...
check offline. We did not resolve the URL, which addresses a named individual's
...
\paragraph{Status is a banner, and the record renders regardless.} The page we
were shown carries a status notice at the top, in red with a cross, which we
read as reporting that the document's validity has lapsed. Beneath it the full
field list renders exactly as it otherwise would.

We do not know what that status means for this class of document, and the
platform does not publish the semantics, so we draw no conclusion about the
particular record. The structural observation stands on its own: a design that
communicates status in a banner and content in a table has separated the finding
from the thing the reader came for. The table is what fills the screen, it is
what looks like the answer, and a reader who is scrolling to find a name --- or
who does not read the language of the banner --- reaches the reassuring part
without passing through the disqualifying one. This is the same failure mode as
Section~\ref{sec:indicators}: people act on the presence of content, not on the
absence of a warning. Where a status can be negative, it has to gate the
disclosure rather than sit above it.
...
verifiable afterwards, so revocation has to be distributed separately, as status
lists that verifiers must fetch and that reintroduce the online dependency the
design existed to remove. This is not a small caveat. It is the hardest unsolved
part of offline credentials, the EU scheme met it too, and a degree that is
rescinded is exactly the case where it bites.
