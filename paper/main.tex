...
The contribution is threefold: a four-layer decomposition of QR payment trust
...
of forty cases, thirty-one of them negative; and an explicit account of what the
design does not achieve and why each residual risk is unreachable from the code
layer. We report two respects in which our own deployment and container fail to
...
Layer~3 is not reachable from the code, but it is not unreachable altogether. It
is reachable from the institutions, and the mechanism is information sharing
about accounts rather than signatures over codes.
...
The analysis applies to any real-time push-payment system in which payments are
irrevocable and initiated from a machine-readable artefact. India's UPI and
Brazil's PIX share both properties, and both have contended with the same
migration of attacker effort from instrument to persuasion. We do not claim
...
will not come from a better signature. It will come from shortening the interval
between an institution recognising a mule account and every other institution
being able to act on it; from allocating liability so that the party best placed
to prevent a loss bears part of it; and, before either, from publishing incident
data so that the prioritisation can be argued from evidence rather than, as
here, from structure.
